...
def getJobs()
{
	def list = ['Linux', 'MacOS', 'Win64_GNU', 'Win64_MSVC', 'Win64_MSVC_dev', 'iOS_OS', 'iOS_Simulator_x86_64', 'iOS_Simulator_arm64', 'FreeBSD']
...
j.with
{
	steps
	{
		...
		phase('Libraries')
		{
			...
			phaseJob(getName('iOS_OS'))
			{
				abortAllJobs(false)
				killPhaseCondition('NEVER')
			}

			phaseJob(getName('iOS_Simulator_x86_64'))
			{
				abortAllJobs(false)
				killPhaseCondition('NEVER')
			}
